...
class BaseManagedPackage(Package):
    ...
    def upgrade(self):
        ...
        if to_replace: logger.warning(f'The following files are selected for (potential) replacement:\n - {"\n - ".join(sorted(to_replace))}')
        if to_remove: logger.warning(f'The following files are selected for removal:\n - {"\n - ".join(sorted(to_remove))}')
