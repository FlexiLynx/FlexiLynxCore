...
def embed_manifest(contents: str, man: base.Manifest) -> str:
...
def extract_manifest(contents: str) -> base.Manifest | None:
    '''Extracts an embedded manifest from a Python file'''
    return base.Manifest.m_from_map(d) if (d := extract_manifest_dict(contents)) is not None else None
