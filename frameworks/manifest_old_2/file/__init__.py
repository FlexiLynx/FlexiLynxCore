...
def extract(data: bytes | Path) -> base.Manifest | None:
    '''
...
# INI stream
_ini_parser = RawConfigParser(delimiters=(': ',), allow_no_value=True)
...
def ini_preprocess(man: base.Manifest) -> dict:
...
def ini_extract(data: bytes | Path) -> base.Manifest | None:
    '''
    ...
    return base.Manifest.m_from_map(ini_postprocess(data))
# JSON stream
def json_preprocess(man: base.Manifest) -> dict:
...
def json_render(man: base.Manifest, compact: typing.Literal[0, 1, 2] = 0) -> bytes:
...
def json_extract(data: bytes | Path) -> base.Manifest | None:
    '''
